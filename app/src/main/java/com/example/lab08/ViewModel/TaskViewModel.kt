...
import com.example.lab08.room.TaskDao
...
class TaskViewModel(private val dao: TaskDao) : ViewModel() {
    ...
    fun deleteTask(task: Task) {
        viewModelScope.launch {
            dao.deleteTask(task)
            _tasks.value = dao.getAllTasks()  // Recargar la lista después de eliminar
        }
    }
    fun updateTask(task: Task) {
        viewModelScope.launch {
            dao.updateTask(task)
            _tasks.value = dao.getAllTasks()  // Recargar la lista de tareas después de actualizar
        }
    }
